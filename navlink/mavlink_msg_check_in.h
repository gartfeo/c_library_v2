...
#define MAVLINK_MSG_ID_CHECK_IN_LEN 8
#define MAVLINK_MSG_ID_CHECK_IN_MIN_LEN 8
#define MAVLINK_MSG_ID_25002_LEN 8
#define MAVLINK_MSG_ID_25002_MIN_LEN 8

#define MAVLINK_MSG_ID_CHECK_IN_CRC 131
#define MAVLINK_MSG_ID_25002_CRC 131
...
#define MAVLINK_MESSAGE_INFO_CHECK_IN { \
    ...
    "CHECK_IN", \
    1, \
    {  { "time", NULL, MAVLINK_TYPE_UINT64_T, 0, 0, offsetof(mavlink_check_in_t, time) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_CHECK_IN { \
...
{
    return mavlink_msg_check_in_pack(system_id, component_id, msg, check_in->time);
}
...
{
    return mavlink_msg_check_in_pack_chan(system_id, component_id, chan, msg, check_in->time);
}
...
{
    return mavlink_msg_check_in_pack_status(system_id, component_id, _status, msg,  check_in->time);
}
...
static inline void mavlink_msg_check_in_send(mavlink_channel_t chan, uint64_t time)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_CHECK_IN_LEN];
    _mav_put_uint64_t(buf, 0, time);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_check_in_send(chan, check_in->time);
#else
 ...
 */
static inline void mavlink_msg_check_in_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint64_t time)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
/**
 * @brief Get field time from check_in message
 *
 * @return [us] Time of applicability of position (microseconds since UNIX epoch).
 */
static inline uint64_t mavlink_msg_check_in_get_time(const mavlink_message_t* msg)
{
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    check_in->time = mavlink_msg_check_in_get_time(msg);
#else
